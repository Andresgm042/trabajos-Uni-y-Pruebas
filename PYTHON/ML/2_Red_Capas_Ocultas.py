...
oculta1 = tf.keras.layers.Dense(units=3, input_shape=[1]) #capa oculta de la red neuronal, units neurona de salida
oculta2 = tf.keras.layers.Dense(units=3) #segunda capa oculta de la red neuronal
salida = tf.keras.layers.Dense(units =1) #capa de salida de la red neuronal, units neurona de salida
modelo = tf.keras.Sequential([oculta1, oculta2, salida]) #modelo de la red neuronal con capas ocultas

modelo.compile(  #compilacion del modelo
    optimizer=tf.keras.optimizers.Adam(0.1), #optimizador de la red neurona
    loss='mean_squared_error' #funcion de perdida, en este caso error cuadratico mediol
)
